...
class ContestView extends React.Component<Props, State> {
   ...
   render() {
      ...
      let tabs = [
         <Tab key={0} label="General information" value={this.createLink()} component={Link} to={this.createLink()} />
      ]

      if (!isNew) {
         tabs = [ ...tabs,
            <Tab key={1} label="Classes" value={this.createLink("classes")} component={Link} to={this.createLink("classes")} />,
            <Tab key={2} label="Problems" value={this.createLink("problems")} component={Link} to={this.createLink("problems")} />,
            <Tab key={3} label="Contenders" value={this.createLink("contenders")} component={Link} to={this.createLink("contenders")} />,
            <Tab key={4} label="Raffles" value={this.createLink("raffles")} component={Link} to={this.createLink("raffles")} />
         ];
      }

      return (
      <div style={{margin:10, display:"flex", flexDirection: "column", flexGrow:1}}>
         <Tabs key="tabs" value={this.state.selectedPath} onChange={this.selectTab}>
            {tabs}
         </Tabs>
