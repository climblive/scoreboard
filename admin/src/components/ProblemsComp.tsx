...
class ProblemsComp extends React.Component<
...
> {
  ...
  getColorName = (problem: Problem) => {
    return problem.colorId
      ? this.props.colorMap.get(problem.colorId)?.name
      : "UNDEFINED";
  };

  getProblemStyle = (problem: Problem) => {
    let color = problem.colorId
      ? this.props.colorMap.get(problem.colorId)
      : undefined;
    if (!color) {
      color = {
        id: undefined,
    ...
    return {
      ...
      alignItems: "center",
      opacity:
        editProblem == undefined || editProblem.id == problem.id ? 1 : 0.08,
  ...
  onFlashBonusChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    this.props.updateEditProblem!(
      "flashBonus",
      flashBonus == 0 ? undefined : flashBonus
    );
  };
  ...
  render() {
    ...
    return [
              ...
              if (editProblem == undefined || editProblem.id != problem.id) {
                return (
                  <li key={problem.id} style={this.getProblemStyle(problem)}>
                    <div style={{ width: 20, fontSize: 16 }}>
                      {problem.number}
                    </div>
                    <div
                      style={{
                    ...
                    >
                      {this.getColorName(problem)}
                    </div>
                    {problem.ticks!.length > 0 && (
                      <Button
                        style={{ color: "inherit" }}
                        onClick={() => this.showTicksDialog(problem)}
                      >
                        {problem.ticks!.length} ticks
                      </Button>
                    )}
                    <div
                      style={{
                ...
                return (
                  <li
                    key={problem.id}
                    style={this.getProblemStyle(editProblem)}
                  >
                    <div style={{ width: 20, fontSize: 16 }}>
                      {problem.number}
                    </div>
                    ...
                    >
                      ...
                      <TextField
                        ...
                        value={
                          editProblem.flashBonus == undefined
                            ? ""
                            : editProblem.flashBonus
                        }
      ...
      />,
      <Dialog key={"tickDialog"} open={this.state.ticksProblem != undefined}>
        <DialogTitle id="confirmation-dialog-title">
          Ticks for problem{" "}
          {this.state.ticksProblem && this.state.ticksProblem!.number}
        </DialogTitle>
        <div
          style={{
            display: "flex",
        ...
        <DialogContent>
          {this.state.ticksProblem &&
            this.state.ticksProblem!.ticks!.map((tick) => {
              let contender = this.props.contenderMap.get(tick.contenderId);
              let compClass = this.props.compClassMap.get(
        ...
        <DialogActions>
          <Button onClick={this.hideTicksDialog} color="primary">
            Ok
          </Button>
        </DialogActions>
      </Dialog>,
