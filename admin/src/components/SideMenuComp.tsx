...
function SideMenuComp({ classes, loggedInUser }: SideMenuCompProps & StyledComponentProps) {
   return (
       <div className={classes!!.sideMenu}>
          ...
          {loggedInUser && <div>
              <Link to="/start"><Button className={classes!!.menuItem}><HomeIcon /><span className={classes!!.menuText}>Start</span></Button></Link>
              ...
              {loggedInUser.admin && <Link to="/locations"><Button className={classes!!.menuItem}><ExploreIcon /><span className={classes!!.menuText}>Locations</span></Button></Link>}
              {loggedInUser.admin && <Link to="/organizers"><Button className={classes!!.menuItem}><PeopleIcon /><span className={classes!!.menuText}>Organizers</span></Button></Link>}
              {loggedInUser.admin && <Link to="/series"><Button className={classes!!.menuItem}><LibraryBooksIcon /><span className={classes!!.menuText}>Series</span></Button></Link>}
          </div>}
          <div style={{margin:"auto 10px 20px 10px", textAlign:"center"}}>
             ...
             <div><pre>v.{Environment.projectVersion}</pre></div>
          </div>
      </div>
   );
}
