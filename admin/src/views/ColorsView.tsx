...
class ColorsView extends React.Component<
  Props & RouteComponentProps & StyledComponentProps,
  State
> {
  ...
  setPopupColor = () => {
    this.props.updateEditColor!(
      this.state.popupType == PopupType.PRIMARY ? "rgbPrimary" : "rgbSecondary",
      this.state.popupColor!
    );
    this.state.popupType = undefined;
    this.setState(this.state);
  };
              ...
              <TableRow>
                ...
                <TableCell className={"icon-cell"} style={{ minWidth: 96 }}>
                  <IconButton
                    ...
                    aria-label="Menu"
                    title="Add color"
                    onClick={this.props.startAddColor}
                  >
                    <AddIcon />
                  </IconButton>
                </TableCell>
                ...
                if (editColor == undefined || color.id != editColor.id) {
                  ...
                  return (
                    <TableRow key={color.id}>
                      ...
                      <TableCell component="th" scope="row">
                        <div style={ColorsView.getColorStyle(color.rgbPrimary)}>
                          {color.rgbPrimary}
                        </div>
                      </TableCell>
                      <TableCell component="th" scope="row">
                        <div
                          style={ColorsView.getColorStyle(color.rgbSecondary)}
                        >
                          {color.rgbSecondary || "None"}
                        </div>
                      </TableCell>
                      <TableCell component="th" scope="row">
                        {color.shared && <PeopleIcon />}
                      </TableCell>
                      <TableCell className={"icon-cell"}>
                        {showEdit && (
                          <IconButton
                            ...
                            aria-label="Menu"
                            title="Edit"
                            disabled={this.isEditable(color)}
                  ...
                  return (
                    <TableRow key={color.id}>
                      ...
                      <TableCell>
                        <div
                          style={ColorsView.getColorStyle(
                            editColor.rgbPrimary,
                            true
                          )}
                          onClick={this.showPopupPrimary}
                        >
                          {editColor.rgbPrimary}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div
                          style={ColorsView.getColorStyle(
                            editColor.rgbSecondary,
                            true
                          )}
                          onClick={this.showPopupSecondary}
                        >
                          {editColor.rgbSecondary || "None"}
                        </div>
                      </TableCell>
                      <TableCell component="th" scope="row">
                        {color.shared && <PeopleIcon />}
        ...
        >
          ...
          <DialogContent style={{ padding: 0 }}>
            <ChromePicker
              ...
              onChange={this.onPopupColorChange}
            />
          </DialogContent>
          <DialogActions>
            ...
            {this.state.popupType == PopupType.SECONDARY && (
              <Button onClick={this.deletePopupColor} color="primary">
                Delete
              </Button>
            )}
            <Button onClick={this.setPopupColor} color="primary">
              Save
            </Button>
          </DialogActions>
        </Dialog>
        <ConfirmationDialog
          open={this.state.deleteColor != undefined}
          title={"Delete color"}
...
function mapDispatchToProps(dispatch: Dispatch<any>) {
  return {
    ...
    updateEditColor: (propName: string, value: any) =>
      dispatch(actions.updateEditColor({ propName: propName, value: value })),
  };
}
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(styles)(withRouter(ColorsView)));
